...
CollectionAPI._requestListener.prototype._requestMethod = function() {
  var method = this._request.method;

  if (this._request.headers['x-http-method-override']) {
    method = this._request.headers['x-http-method-override'];
  }
...
CollectionAPI._requestListener.prototype._handleRequest = function() {
  ...
  if (!self._requestMethodAllowed(self._requestMethod())) {
    return self._notSupportedResponse();
  }

  switch (self._requestMethod()) {
    case 'GET':
...
CollectionAPI._requestListener.prototype._getRequest = function(fromPutRequest) {
  ...
  self._server._fiber(function() {
    ...
    try {
      ...
      if(!self._beforeHandling('GET',  self._requestPath.collectionId, records, self._requestAuthToken)) {
        if (fromPutRequest) {
...
CollectionAPI._requestListener.prototype._deleteRequest = function() {
  ...
  self._server._fiber(function() {
    try {      
      if(!self._beforeHandling('DELETE', self._requestPath.collectionId, self._requestCollection.findOne(self._requestPath.collectionId), self._requestAuthToken)) {
  ...
  self._request.on('end', function() {
    self._server._fiber(function() {
      try {
        ...
        if(!self._beforeHandling('POST', obj, self._requestAuthToken)) {
